...
export function Dashboard() {
  ...
  async function loadData() {
    ...
    if (loadedCustomers.length > 0) {
      setNewCustomerId((prev) => prev || loadedCustomers[0].id);
      setNewEmail((prev) => prev || loadedCustomers[0].email || "");
    }
  ...
  async function saveCustomer() {
    ...
    const created = insert.data as Customer;

    setCustomers((prev) => [created, ...prev]);
    setNewCustomerId(created.id);
    setNewEmail(created.email || "");
  ...
  async function saveNewInvoice() {
    ...
    const invoiceInsert = await supabase
      ...
      .insert({
        ...
        status: "offen",
        vat_rate: newVatRate,
        net_amount: calcNet(validItems),
        vat_amount: calcVat(validItems, newVatRate),
        gross_amount: calcGross(validItems, newVatRate),
  ...
  function updateNewItem(index: number, patch: Partial<InvoiceItem>) {
    setNewItems((prev) =>
      ...
      <section style={styles.main}>
        ...
        <div style={styles.grid}>
            ...
            {newItems.map((item, index) => (
              <div key={index} style={styles.newItemGrid}>
                ...
                <input
                  placeholder="Menge"
                  value={item.qty}
                  onChange={(e) => updateNewItem(index, { qty: Number(e.target.value || 0) })}
                  style={styles.input}
                />
                <input
                  placeholder="Einheit"
          ...
          <div style={styles.panel}>
            ...
            {selectedInvoice ? (
              <>
                <div style={{ marginTop: 18, fontWeight: 800, fontSize: 18 }}>Details</div>
